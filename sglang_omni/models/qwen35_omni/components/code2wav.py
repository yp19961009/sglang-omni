...
"""Qwen3.5 public_v1 codec decoder, adapted from the vLLM reference."""
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.pos_embed_type == "rope":
            ...
            raise NotImplementedError("conformer pos_embed_type not implemented")
...
class DAC(nn.Module):
    ...
    def _decode_inner(self, codes: torch.Tensor):
        if self.quantizer is not None:
            ...
            code_embs = [
                emb(code) for emb, code in zip(self.code_embeddings, code_list)
            ]
...
def load_qwen3_omni_next_dac_from_config(
    ...
    tranformer_config_quantizer = ModelArgs(
        ...
        rope=(
            hps.post_module.args.rope if hasattr(hps.post_module.args, "rope") else None
        ),
